...
H2SPEC_BADGE = ROOT / "h2spec-badge.json"
CERTS = BUILD / "certs"

EXPECTED_HPACK_CHECKS = (
    "python-hpack encode -> mojo decode (46 blocks, shared dynamic table)",
    "mojo encode -> python-hpack decode (46 blocks)",
    "compression ratio sanity (mojo 2821B vs python 2823B, within 25%)",
    "dynamic table size updates mid-stream (python-hpack -> mojo)",
    "multibyte UTF-8 header values roundtrip (python -> mojo)",
)
EXPECTED_H2_CHECKS = (
    "parse hyperframe-built frames (all 10 types, 12 cases)",
    "byte-identical frame serialization vs hyperframe (12 cases)",
    "incremental decode at every split point (15 plans)",
    "one-byte chunks preserve 7 hyperframe frames",
    "coalesced input preserves 7 hyperframe frames",
    "seeded fragmentation of a 16 KiB hyperframe DATA frame",
    "incomplete hyperframe suffix is retained without early output",
    "connection startup accepts hyper-h2 bytes at every split point (76 plans)",
    "one-byte startup feeds preserve all 75 hyper-h2 bytes",
    ...
    "process_frame preserves HPACK state and bounds live state across 4096 retired hyper-h2 streams",
    ...
    "mojo client vs hyper-h2 server (200KB echo, strict validation)",
    "mojo client reassembles HEADERS+CONTINUATION (32KB header block) and padded DATA",
    "hyper-h2 client vs mojo server (50KB echo, strict validation)",
    "h2spec cleartext full run (146 tests, 146 passed, 0 skipped, 0 failed)",
)
EXPECTED_H2_TLS_CHECKS = (
    ...
    "h2spec TLS full run (146 tests, 146 passed, 0 skipped, 0 failed)",
)
EXPECTED_CHECKS = {
    "hpack": EXPECTED_HPACK_CHECKS,
    "h2": EXPECTED_H2_CHECKS,
    "h2_tls": EXPECTED_H2_TLS_CHECKS,
}

RESULTS: dict[str, list[tuple[str, bool, str]]] = {}
H2SPEC_SUMMARIES: dict[str, tuple[int, int, int, int]] = {}
US = "\x1f"
MOJO_RUN: list[str] = []
...
def section_h2spec(tmp: Path):
    ...
    for use_tls in (False, True):
        ...
        try:
            ...
            if msum:
                H2SPEC_SUMMARIES[mode] = tuple(map(int, msum.groups()))
...
def section_result_summary(
    results: dict[str, list[tuple[str, bool, str]]], section: str
) -> tuple[int, int, bool]:
    """Count one section and require every registered row exactly once."""
    expected_names = EXPECTED_CHECKS[section]
    rows = results.get(section, [])
    indexed: dict[str, list[bool]] = {}
    for name, ok, _ in rows:
        indexed.setdefault(name, []).append(bool(ok))
    ...
    for name in expected_names:
        ...
        if len(outcomes) == 1 and outcomes[0]:
            passed += 1
    valid = (
        len(rows) == len(expected_names)
        and set(indexed) == expected
        and all(len(indexed[name]) == 1 for name in expected)
    )
    return passed, len(expected_names), valid


def result_summary(
    results: dict[str, list[tuple[str, bool, str]]],
) -> tuple[int, int, bool]:
    """Count registered checks and reject incomplete result sets."""
    passed = 0
    total = 0
    valid = set(results) == set(EXPECTED_CHECKS)
    for section in EXPECTED_CHECKS:
        section_passed, section_total, section_valid = section_result_summary(
            results, section
        )
...
def write_report() -> bool:
    passed, total, valid = result_summary(RESULTS)
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    verdict = (
        "checks passed" if valid else "registered checks passed; results incomplete"
    )
    lines = [
    ...
    for section, rows in RESULTS.items():
        if section in EXPECTED_CHECKS:
            p, section_total, _ = section_result_summary(RESULTS, section)
        else:
            p = 0
            section_total = len(rows)
        title = SECTION_TITLES.get(section, f"`{section}`")
        lines += ["", f"## {title}: {p}/{section_total}", "",
                  "| Check | Result |", "|---|---|"]
    ...
    REPORT.write_text("\n".join(lines))
    print(f"\ncompliance: {passed}/{total} {verdict}")
...
def h2spec_badge_payload(
    summaries: dict[str, tuple[int, int, int, int]],
) -> dict[str, object]:
    """Build a Shields endpoint payload from the two h2spec summaries."""
    messages = []
    missing = False
    failed = False
    incomplete = False
    for mode in ("cleartext", "TLS"):
        summary = summaries.get(mode)
        if summary is None:
            messages.append(f"{mode} unavailable")
            missing = True
            continue
        total, passed, skipped, failures = summary
        messages.append(f"{mode} {passed}/{total}")
        failed = failed or failures > 0
        incomplete = incomplete or skipped > 0 or passed != total
...
def h2spec_badge_json(summaries: dict[str, tuple[int, int, int, int]]) -> str:
    """Serialize the endpoint payload in a stable form."""
    return (
        json.dumps(h2spec_badge_payload(summaries), indent=2, sort_keys=True)
        + "\n"
    )


def write_h2spec_badge():
    H2SPEC_BADGE.write_text(h2spec_badge_json(H2SPEC_SUMMARIES))
    print(f"report: {H2SPEC_BADGE.relative_to(ROOT)}")
...
def write_html_report() -> bool:
    ...
    for section, rows in RESULTS.items():
        if section in EXPECTED_CHECKS:
            p, section_total, section_valid = section_result_summary(RESULTS, section)
        else:
            p = 0
            section_total = len(rows)
            section_valid = False
...
def main() -> int:
    ...
    return 0 if markdown_ok and html_ok else 1
